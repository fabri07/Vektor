...
**Estado de ejecución (actualizado 2026-09-10).** El encabezado de arriba describe la planificación original; esto describe lo entregado, y manda sobre él.
...
| E5 — verificación operativa (H16/H10) | **Parcial.** El barrido de relecturas ya deja traza durable de cada ejecución (`job_runs`, migración `20260909_0001`) y `scripts/diag_job_runs.py` reporta ejecuciones, fallas y **cadencia observada** con una consulta read-only. Falta lo que no se puede leer desde el repositorio: estado del despliegue del servicio de Beat, si `CELERY_QUEUES` está sobrescrita en el worker, y las cuatro flags de rollout por servicio. Una ejecución registrada prueba que alguien consumió la task, no que la publicó Beat. |
| E6a — envíos entre formatos + códigos externos de maestros | **Entregada.** El cobro de envío salió del closure multihoja y lo comparten los dos caminos, con compuerta de paridad sobre efectos persistidos; el 422 se retiró recién con esa paridad probada. Código externo de maestros con índice único parcial desde el inicio. |
| E6b — deduplicación por clave fuerte | **Entregada.** Ver el detalle más abajo. |
| E6c — dedup acotada, límites y ejecución duradera | **Entregada** en sus tres bloques (1 acotar, 2 límites/fórmulas, 3 asíncrono), con F0 medido antes de tocar nada. Pendiente de habilitación: migraciones en Neon y la compuerta por tenant. |
| E7a | **No se compromete.** Evaluada con datos; la brecha medida no la justifica. Se entregó **E7a-lite** — ver más abajo. |
| E7b/E7c | Sin empezar. E7b requiere E7a, que no va. Los diagnósticos read-only de E7c son motor actual y pueden adelantarse. |
| E8 — el documento que sólo deja pendientes | **Entregada.** El confirm de foto/PDF/texto terminaba en 422 y el rollback se llevaba puestas las capturas que ese camino acababa de hacer en «Otros»: el archivo se procesaba y en la bandeja no quedaba nada. «Otros» cuenta como manejo válido **sólo en documentos** —en planilla puede tapar un mapeo que falló—, el resultado dice «pendientes de revisión» en vez de presentarse como importado, y las capturas llevan ancla por **ocurrencia** `(archivo, contexto, ordinal)`, nunca por texto: dos renglones idénticos son dos ventas. Verificada en los dos recorridos (sincrónico y asíncrono, con el resultado del intento consultable), contra Postgres real para durabilidad tras el commit, caída/reintento y relectura. |
| E8b — el documento fija el convenio numérico | **Entregada.** El camino de documentos era el único que leía montos fuera de la política de E4: `$12.500` no se podía interpretar sin convenio y la línea desaparecía sin rastro. El documento pasa a ser la unidad que fija la escala, con la MISMA regla de evidencia que una columna; lo que aun así no se puede interpretar se conserva en «Otros» con su motivo y su valor original. `$0` deja de ser indistinguible de la ausencia. El texto sin ningún token de monto sigue sin materializarse. |
| E8c — migraciones idempotentes | **Entregada.** Un deploy abortó con `DuplicateColumn` porque el esquema de Neon estaba por delante de `alembic_version`. Las 7 migraciones del programa guardan cada creación detrás de `sa.inspect`, siguiendo la convención que `20260806_0001` ya documentaba. Compuerta en PostgreSQL real que reproduce la deriva (stamp atrás, esquema intacto) y re-aplica. |

Lo entregado son correcciones sobre el motor actual. La compuerta de E7a —el refactor grande— **se evaluó con datos y no se compromete**: la divergencia medida entre los dos caminos del importador eran dos defectos de una línea cada uno, y reemplazar el motor no es la respuesta proporcionada a eso. Se entregó E7a-lite en su lugar.
...
**Implementado (2026-09-10), y las precisiones que el criterio no fijaba.** El dominio vive en `backend/app/domain/operation_identity.py`; el candado y las decisiones, en `application/services/operation_identity_service.py`. Seis cosas que hubo que decidir al bajarlo, todas del lado exigente:

1. **Un `external_operation_id` sin sistema de origen declarado NO es clave.** El "1024" de un sistema y el de otro son dos operaciones. Efecto lateral valioso: eso también protege de la heurística de encabezados — que confunda un número de fila con un ID externo no puede producir un duplicado falso.
2. **El emisor de una venta propia es el negocio, no el cliente.** El cliente es el RECEPTOR. Meterlo en la clave haría que cambiarle el cliente a una factura ya importada la dejara entrar de nuevo.
3. **El emisor se identifica por CUIT/CUIL, nunca por nombre.** Una compra sin el CUIT del proveedor no produce clave: el nombre se escribe de mil formas y dos empresas se llaman igual.
4. **Tipo y serie se exigen según el formato** (`FORMATOS_DE_COMPROBANTE`), no como opcionales. Un tipo que no está en el catálogo no produce clave, porque no sabemos qué lo distingue. Se lee `PPPP-NNNNNNNN` cuando el punto de venta viene adentro del número —eso es parsear el formato, no adivinar— y si la columna y el número se contradicen, tampoco hay clave.
5. **La identidad de línea no se deriva.** Sin un `external_line_id` explícito, la unidad de comparación es el **documento completo**, con sus renglones repetidos preservados y ordenados (reordenar la planilla no lo vuelve otro documento). Derivarla de "producto + ordinal" produce una clave que cambia sola.
6. **La huella de contenido va sobre los valores EFECTIVOS**, resueltos con las mismas funciones del importador (`resolve_line_amount`, los parsers de E4), no sobre el texto de las celdas ni sólo sobre los campos mapeados. Versionada (`k1:` / `c1:`): sin la versión adentro, cambiar la forma se vería como "todo es nuevo" sin ninguna señal. Límite declarado: si la huella predicha divergiera de lo persistido, el efecto sería un **conflicto** que va a revisión — nunca un salteo silencioso.

**Lo que garantiza la implementación, y con qué se prueba.** El candado es el UNIQUE `(tenant_id, identity_key)` y se toma con `INSERT ... ON CONFLICT DO NOTHING RETURNING` **antes** de escribir efectos: dos confirmaciones simultáneas del mismo comprobante aplican una sola vez (probado contra Postgres real; con una comprobación en memoria el test se pone rojo). Identidades, vínculos y efectos comparten la transacción del confirm. Las reclamaciones que no produjeron efecto se **devuelven** al cierre, para que el archivo corregido se pueda reimportar. La liberación es por efecto revertido y sólo cuando la identidad queda sin ninguno: una reversión parcial NO la suelta.

**Y lo que NO hace, escrito para que no se lea como una promesa:** la importación en lote desde «Otros» no resuelve conflictos (los manda a `needs_manual`); y los campos de identidad **no se sugieren solos** salvo el número de comprobante, porque el reconocedor de encabezados hoy lee «Punto de venta» como monto de venta.

**El camino legacy sin `mapping_contexts`, cerrado (2026-09-11).** Era el límite que E6b dejó declarado: ese camino lee por keyword, no tiene mapeo del que derivar la identidad de un comprobante, y por lo tanto persistía ventas y gastos que nadie podía reconocer como repetidos. **La salida no es adivinar la clave** —una identidad falsa hace que Véktor descarte plata real, que es peor que el duplicado—, así que se rechaza el import con un 422 que manda a releer.

El alcance se midió antes de escribir el bloqueo, y es lo que lo hace barato: **ningún camino de parseo actual produce un summary con operaciones y sin contextos** (los únicos sin contextos son los de archivos vacíos), y en producción **cero archivos** están en ese estado —6 vivos, ninguno alcanzado—. O sea que alcanza sólo a summaries persistidos antes del mapeo universal.

El bloqueo es acotado a lo que *iría a escribir operaciones*: no toca los archivos vacíos, ni los documentos que rutean a «Otros», ni los imports que sólo traen productos, ni la tabla suelta —que no pasa por esa rama y sí planifica identidades—. El predicado vive en `domain/legacy_import_guard.py` y **espeja el despacho real** de `insert_confirmed_data`; si divergiera protegería un camino distinto del que escribe.

Cambio de contrato deliberado, con su costo: la compuerta de E4 que exigía **200** importando ventas por ese camino pasó a exigir el 422, y la garantía que ejercía —`ignore` respetado y `"8.400"` leído como 8400— se mide ahora sobre el bucket de PRODUCTOS, que sigue habilitado y pasa por `preparar_filas_de_hoja`, la misma función. Tres mutaciones, tres conjuntos de pruebas distintos: quitar el filtrado de ignoradas mata la de `ignore`; sacar el convenio numérico mata la del convenio; apagar el bloqueo mata cuatro del rechazo.

Y se corrigió el mensaje antes de commitearlo: decía "volvé a leer el archivo y confirmalo de nuevo", y **medido, la relectura no deja nada para reconfirmar — importa ella misma** con el resumen fresco (que sí trae contextos, y por eso pasa por la identidad fuerte). Mandar a reconfirmar habría sido mandar a un rechazo idéntico, porque el resumen guardado sigue siendo el viejo.

**Costo medido** (Postgres local, mismo agrupador que la compuerta): un archivo sin columnas de identidad paga **0 statements extra** (29 → 29 en 300 productos + 300 ventas); uno con comprobante completo paga **3 fijos** (57 → 60 en 300 compras, con un comprobante distinto por fila). Queda como compuerta permanente en `test_ingestion_statement_budget_pg.py`.

**E6a implementado (2026-09-10) — envíos entre formatos, y código externo de maestros.**

*Mitad A (H04).* El rechazo total del archivo plano con costos de compra se reemplazó por uno específico. Los tres defectos que lo justificaban están arreglados: `cobrar_envios_de_la_hoja` dejó de ser un closure del camino multi-hoja (ahora es una función de módulo con `EntornoDeEnvios` explícito), el camino plano conserva la **clave** de su hoja —la descartaba, y por eso las decisiones de costo/envío se validaban, se aceptaban y se ignoraban— y los avisos de costo llegan a `counts`. **Una tabla única SÍ agrupa por comprobante**: lo determinante son los identificadores de la fila (proveedor + número), no que exista una hoja aparte. Lo único que queda rechazado son decisiones sobre mapeos que no declaran su hoja. La compuerta es de **paridad de efectos persistidos** entre formatos (`test_paridad_tabla_vs_hoja`): gastos, caja, costos, stock, envío cobrado una sola vez, con varios comprobantes, filas reordenadas, reintento y borrado. Medido: multihoja 58 → 58 statements (la extracción no cambió nada), tabla suelta 69 → 70 y de **0 a 30 envíos** en 300 compras.

*Mitad B (F-I).* `external_code` + `external_source` + `external_code_key` en `customers`/`suppliers`/`products` (mig `20260910_0002`), con **índice único parcial desde el principio**: las columnas nacen vacías y sin backfill, así que no hay colisiones que prevalidar, y una clave fuerte sin UNIQUE no impide que dos imports simultáneos creen dos entidades con el mismo código. El predicado excluye las bajas porque el índice tiene que ver lo mismo que la búsqueda (`list_for_dedup` / `is_active`); si no, una entidad dada de baja rompe el insert con un error indiagnosticable. Reglas: ámbito tenant + tipo de entidad + **sistema de origen** declarado; **los ceros a la izquierda no se recortan** (un código es una cadena — la diferencia deliberada con `document_series`, que sí es un entero). Se cablea a `identity_resolution` (maestros) y a las columnas normalizadas de `products`, **sin mezclar contratos**: la huella de contenido de `operation_identity` no define equivalencia de maestros.

*La contracara, que es la mitad que faltaba.* Que dos filas sean la misma entidad no dice cuál versión de un teléfono vale. `has_user_edits` (mig `20260910_0003`) lo marca el PATCH y vuelve el import **aditivo** sobre esa ficha: completa lo vacío, no pisa lo cargado, y reporta lo preservado. Aditivo y no bloqueo: corregir un teléfono no puede impedir para siempre que el padrón traiga el email.
...
**E6c-1 — deduplicación acotada al archivo (2026-09-10).** `_load_import_fingerprints` traía todas las huellas del tenant; ahora se enumeran las anclas que el import puede consultar —derivables de `(archivo, contexto, índice)`— y se preguntan por **lotes de 2.000** (`huellas_presentes`), bien debajo del tope de parámetros de PostgreSQL. `HuellasDelArchivo` recuerda **qué preguntó**, para poder distinguir "no está en la base" de "nunca la preguntamos": sin esa distinción, una precarga acotada volvería a cobrar un flete ya cobrado en cada re-confirmación. Lo que queda fuera del alcance —el cargo de envío, cuya ancla incluye el monto— paga un `SELECT` y se cachea; son uno por comprobante, no por fila. **No es la protección contra concurrencia y nunca lo fue**: la escritura sigue con `ON CONFLICT DO NOTHING` y la exclusión la da el lease.

| filas | historial | pico antes | pico después | stmts | tiempo antes | tiempo después |
|---:|---:|---:|---:|---:|---:|---:|
| 100 | 0 | 4,6 MB | 4,7 MB | 57 → 57 | 1,19 s | 1,23 s / 6,17 s |
| 100 | 100.000 | **32,3 MB** | **0,8 MB** | 57 → 57 | 2,53 s | 0,76 s / 0,89 s |
| 1.000 | 100.000 | 32,6 MB | 8,5 MB | 100 → 101 | 6,11 s | 4,29 s / 4,81 s |
| 5.000 | 0 | 19,3 MB | 21,3 MB | 124 → 131 | 15,72 s | 17,07 s / 20,67 s |
| 5.000 | 100.000 | 36,3 MB | 21,2 MB | 124 → 131 | 18,11 s | 17,61 s / 25,53 s |

**Sobre la latencia, con la salvedad que corresponde:** se muestran las DOS corridas posteriores porque en esta máquina el tiempo de pared **no es reproducible** — la misma celda (100 filas / 0 historial) dio 1,23 s y 6,17 s. Las dimensiones estables son statements y memoria pico. La única mejora de latencia que se repite en las dos corridas es la del caso dominado por la historia: **2,53 s → 0,76-0,89 s** en 100 filas contra 100.000 huellas. El resto está dentro del ruido y no se afirma como mejora ni como regresión; medirlo en serio pide una máquina dedicada.

Costo aceptado: **+1 statement** en 1.000 filas y **+7** en 5.000 — O(filas/2.000), acotado. La compuerta (`test_las_huellas_se_consultan_acotadas_al_archivo`) mira la FORMA del SQL y no el conteo: la primera versión contaba statements y **volver al `SELECT` sin filtro no la ponía en rojo**, porque una query que trae 50.000 filas y una que trae 200 cuentan igual.

**E6c-2 — límites verificables y fórmulas (2026-09-10).** Siete topes (bytes, expandido, filas, columnas, hojas, celdas, tiempo) más el archivo dañado, todos con motivo específico y acción concreta, verificados en `parse_uploaded_content` — el único punto por el que pasan los cinco caminos que parsean; los tres del worker leen de S3 y nunca tocaron el chequeo del endpoint. Se rechaza el archivo ENTERO: truncar produce libros que no cuadran sin que nadie sepa que están incompletos. El tamaño expandido se cuenta descomprimiendo por bloques y cortando al pasarse, no sumando los metadatos del ZIP; un ZIP cuyo contenido no coincide con lo que declara se rechaza como dañado en vez de salir como error genérico. El tope de tiempo se consulta DENTRO del bucle de filas (`read_only` es perezoso, así que cortar el bucle corta el trabajo) y **en la primera llamada** — antes controlaba cada 512, así que un archivo de menos de 512 filas no consultaba el reloj ni una vez.

Y `data_only=True` devolvía `None` tanto para una celda vacía como para una fórmula sin resultado cacheado, así que un `.xlsx` generado por script entraba con todas sus columnas calculadas leídas como vacías —sin fallar, sin avisar, sin quedar en «Otros»—. Ahora los tres estados se distinguen con una segunda lectura (`data_only=False`), que se paga sólo si hubo `None`. Límite declarado: detecta la fórmula SIN resultado, **no** la que tiene un resultado viejo; cuando no se puede verificar se dice, en vez de devolver cero.

**E6c-3 — ejecución duradera y asíncrona (2026-09-10).** `POST /ingestion/files/{id}/imports` registra la solicitud congelada y su orden de ejecución **en la misma transacción** (`import_attempts` + `import_outbox`), devuelve 202 con un id consultable, y `GET /ingestion/imports/{attempt_id}` lo sigue. `/confirm` queda operativo; la compuerta es `ASYNC_IMPORT_ROLLOUT_TENANT_IDS` y gatea **sólo el registro de solicitudes nuevas** — el publicador y el recuperador siguen corriendo para todos, porque apagarlos dejaría sin ejecutar intentos ya commiteados. La exclusión es el lease POR ARCHIVO que ya usaba `/confirm`: el ejecutor llama al mismo endpoint, así que las dos rutas no pueden importar a la vez. El transporte no promete "exactly once" —el publicador entrega y después marca, porque al revés perdería órdenes— y lo que impide la doble ejecución es el token del intento. Runbook: `docs/runbooks/async_import_rollout.md`.

**Regla del cliente, no negociable:** ante un timeout del POST el frontend **nunca** cae a `/confirm`. Un timeout no dice que la petición no llegó, dice que no se sabe; disparar el confirm encima importaría dos veces. Reintenta con la MISMA clave de petición, que se persiste ANTES de mandar (en memoria, un reload durante el timeout la perdería y el reintento crearía un segundo intento). El único caso donde `/confirm` es seguro es un **404**: la ruta no está habilitada, o sea que el servidor respondió sin mirar el cuerpo.

**Revisión externa del recorrido asíncrono (2026-09-10), y los cinco arreglos.** Una revisión sobre `635c9ae7..7de4a461` reprodujo cuatro defectos con pruebas ejecutables y señaló un quinto por inspección. Los cinco eran reales y dos contradecían comentarios escritos en el propio código:

1. **Se podía importar una revisión distinta de la confirmada.** El intento congelaba `content_hash`, que es el sha256 de los BYTES; una relectura no los toca —reescribe `parsed_summary_json` y sube `ingestion_version`—, así que el guard que decía proteger de una relectura era exactamente ciego a ella. Reproducido: registrar $3.000 y persistir $27.000. Arreglo en dos mitades: verificar la revisión congelada **con el lease ya tomado**, y hacer que `reread/apply` **respete ese lease** (no miraba `processing_status` en absoluto, así que sin esto la verificación sólo angostaba la ventana).
2. **La recuperación reabría la ventana de pérdida de publicación.** Commiteaba el `PENDIENTE` y después llamaba al broker; si esa llamada fallaba —y un broker caído es un motivo HABITUAL de que haya huérfanos— el intento quedaba sin orden pendiente, invisible para el publicador y sin ejecución para siempre. Ahora la orden se re-arma en la MISMA transacción y la recuperación no habla con el broker.
3. **Una caída después de guardar los efectos informaba fracaso.** `confirm_file` no commitea (lo hace el caller), así que el cierre del intento se movió a la misma transacción que los efectos: o commitean las dos cosas o ninguna. Cierra de paso la otra mitad —perder el lease ahora revierte los efectos en vez de dejarlos sin dueño— y obligó a compensar el `IMPORTING` que `acquire_import_lease` commitea a propósito, que si no dejaba el archivo trabado con sus efectos revertidos.
4. **El recuperador escribía sin fencing.** `liberar_huerfanos` hacía SELECT + mutación ORM: un ejecutor podía renovar, terminar o ser reemplazado en el medio y quedar pisado. Ahora es un UPDATE condicionado por fila con `rowcount`, el mismo patrón CAS que sus dos vecinos ya usaban. Un `lease_token` nulo se trata como estado corrupto recuperable (`IS NULL`), porque `= NULL` no matchea nunca y el intento quedaría "importando" para siempre.
5. **Una clave de petición podía devolver el intento de OTRO archivo.** El `file_id` no está en el cuerpo —es un parámetro de la ruta— así que la huella del payload no distinguía dos archivos con los mismos mapeos: el segundo recibía el intento del primero y nunca se importaba. No es un duplicado, es una pérdida silenciosa. Ahora el `file_id` entra en la comparación (no en la huella, para no invalidar lo que está en vuelo).

Las cinco pruebas del revisor se adoptaron al repo: eran la única compuerta de estos caminos, y cuatro de ellos ninguna prueba los cruzaba.

**Y el mutation testing corrigió dos de los arreglos, no sólo los confirmó.** De las seis mutaciones, cuatro murieron a la primera y **dos sobrevivieron**, señalando defectos en el trabajo propio:

- La prueba del outbox era **vacua**: el registro en tests no llega a marcar la orden como publicada, así que la orden seguía pendiente por sí sola y la prueba pasaba con el arreglo y sin él. Se corrigió marcándola entregada primero, que es lo que pasa siempre que un ejecutor llegó a tomar el intento.
- El fencing por `lease_token` **no lo justifica ningún escenario**: los tres casos que preocupaban mueven `status` o `lease_expires_at`, y esas dos condiciones ya los excluyen. Borrar la comparación de token no rompía nada, y traía un borde propio (`lease_token = NULL` no matchea nunca en SQL, así que un `EJECUTANDO` con token nulo no se habría podido liberar jamás). Se sacó.
- Peor: las dos pruebas de fencing **no ejercitaban la carrera** —cambiaban el estado ANTES de llamar al recuperador, o sea que probaban el filtro del SELECT— y pasaban con la escritura sin condiciones. La carrera real se reproduce con **dos recuperadores concurrentes** sobre el mismo huérfano: con la mutación del objeto ORM lo liberan los dos y se encola dos veces el mismo trabajo. Esa es la prueba que queda, y la mutación muere en 1,3 s.

El arreglo del punto 3 también destapó algo que ninguna prueba veía: `acquire_import_lease` **commitea** el `IMPORTING` a propósito, así que revertir los efectos dejaba el archivo trabado en "importando" con sus libros vacíos. Hubo que compensar el lease del archivo, y **en sesión propia** — hacerlo sobre la sesión que acaba de fallar cuelga el proceso.

**E7a evaluada con datos, y por qué salió "lite".** La compuerta de E7a pedía una brecha medida que las correcciones localizadas no cierren. Medido sobre 13 formas de archivo por los dos caminos del importador, las divergencias reales eran **dos**, las dos localizadas:

1. Un gasto sin monto utilizable **desaparecía sin rastro y sin aviso** en el camino multihoja, mientras el de tabla suelta lo capturaba en «Otros». El mismo archivo perdía filas o no según estuviera armado como hoja o como tabla.
2. `montos_ambiguos` sólo lo publicaba el camino plano, así que un archivo cuya columna de montos mezcla escalas no movía el contador que discrimina el mensaje de import vacío.

Dos defectos de una línea cada uno no justifican reemplazar el motor: justifican arreglarlos y **poner la compuerta que los habría visto**. E7a queda sin comprometerse; lo entregado es E7a-lite:

- Los dos defectos, cerrados y verificados por mutación.
- **Paridad exhaustiva por contadores** (`app/tests/services/test_paridad_tabla_vs_hoja.py`): el mismo contenido por los dos caminos, comparando TODOS los contadores por diferencia —no contra una lista de claves esperadas, así que un contador nuevo entra al alcance solo—. Única excepción declarada: los contadores que llevan uuids de entidades recién creadas se comparan por cardinalidad. Se compara por contadores y no sólo por efectos persistidos porque una fila que se pierde en silencio no mueve ningún efecto, justamente porque no produjo ninguno.
- **Snapshot de capacidades efectivas en el intento** (`import_attempts.capabilities_json`, migración `20260910_0005`): las cuatro compuertas de rollout que cambian qué se persiste se congelan cuando el usuario confirma, y el ejecutor las verifica antes de escribir. Alcanzable y grave: la api y el worker son dos servicios con entornos propios que Railway redespliega en paralelo, así que alguien podía confirmar viendo el motor de costos de compra encendido y el worker escribir los costos con el motor apagado. Se verifica en vez de imponer —imponer exigiría overrides en los ocho lugares donde se leen las compuertas, y una compuerta nueva quedaría afuera del override afirmando estar congelada—; el precio declarado es que girar una llave invalida los intentos en vuelo de ese tenant, con un mensaje accionable y sin nada escrito.
