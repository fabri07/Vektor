...
**Y lo que NO hace, escrito para que no se lea como una promesa:** la importación en lote desde «Otros» no resuelve conflictos (los manda a `needs_manual`); el camino legacy —summaries sin `mapping_contexts`— no calcula ni consulta identidad, así que una fila que entre por ahí puede duplicar algo que otro archivo cargó con identidad; y los campos de identidad **no se sugieren solos** salvo el número de comprobante, porque el reconocedor de encabezados hoy lee «Punto de venta» como monto de venta.
