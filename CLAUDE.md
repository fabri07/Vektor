...
- **Reforma de Proveedores (marca ≠ proveedor):** los catálogos de productos ya NO crean proveedores — la marca queda en `Product.custom_fields["marca"]`. Una compra sin proveedor informado se agrupa en un único **sentinela "No identificado"** por tenant (flag `custom_fields["_sentinel"]`, índice único parcial, helper centralizado `is_sentinel_value`/`Supplier.is_sentinel` en `models/supplier.py`). Campos de ficha: `last_name`/`cuil`/`payment_method` (persona o razón social). `inventory_movements.supplier_id` alimenta `GET /suppliers/{id}/products` (`InventoryRepository.products_purchased_from_supplier`: última compra/cantidad/precio unit). Frontend: detalle a **página `/suppliers/[id]`** (no modal), nombre navegable, mailto/WhatsApp, helpers en `frontend/src/lib/suppliers.ts`.
  - **Cambio 4 (en curso, `docs/plans/conservacion-y-acceso-datos-negocio.md`):** mapear "Proveedor — Nombre" (`supplier:name`) con `PRODUCT_SUPPLIER_LINKS_ROLLOUT_TENANT_IDS` apagado ya NO se degrada en silencio a "marca" en los dos caminos donde el usuario ELIGE el mapeo en la misma llamada — `POST .../confirm` y `POST .../reread/preview` rechazan con 422 ANTES del lease (`domain/import_capabilities.contexts_mapping_disabled_supplier_link`). El downgrade silencioso de `_add_product` (`counts["supplier_link_not_enabled"]`) **sigue vigente a propósito** como red para el caso todavía no cubierto: una relectura que NO resubmite mapeo y replica uno aprendido de cuando el flag estaba prendido (no pasa por `body.column_mappings` de ninguno de los dos endpoints). `GET /ingestion/capabilities` (nuevo, tenant-aware — a diferencia del estático `GET /ingestion/field-catalog`) expone `import_capabilities.capacidades_efectivas()` para que el mapeador pueda deshabilitar el destino antes de confirmar. Falta: comprobante de importación persistido (`pipeline_events.detail`/`DataRepairRun.details_json`), cerrar el gap de la relectura sin resubmisión, y wiring del frontend (mapper + historial).
