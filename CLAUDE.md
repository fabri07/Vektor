...
- **Interpretación por COLUMNA, nunca por celda (E4):** un valor de negocio se lee con el convenio de su columna, resuelto una sola vez. **Números** (`domain/numeric_parsing.py`): un nativo de Excel no se reinterpreta; el convenio decimal sale de la columna (`inferir_convenio`) y, elegido, la celda TODAVÍA tiene que tener esa forma —grupos de miles de tres dígitos, un solo separador decimal— o no se interpreta (`MOTIVO_INCOMPATIBLE`; sin esa validación `"12.50"` en una columna AR entraba como 1250). **Fechas** (`domain/date_parsing.py`): el orden día/mes lo decide la columna (`inferir_convenio_de_fecha`) — una fecha imposible como mm/dd fija el orden de todas; sin señal manda la convención AR; una columna contradictoria manda lo ambiguo a revisión. Incluye seriales de Excel (ventana 1990-2099). **Cantidades**: enteras y no negativas, nunca truncadas ni convertidas en 1 — una celda escrita e ilegible manda la fila a "Otros" con su motivo; una celda VACÍA sí vale 1. El cableado es una pasada por hoja (`preparar_filas_de_hoja` = sacar columnas ignoradas → normalizar números → normalizar fechas), que consumen los tres caminos del importador **y el preview de costos**: si el preview leyera distinto, la pantalla mostraría un costo que el import no produce. Las celdas resueltas quedan como valor NATIVO (`Decimal`/`datetime`), así que los ~35 lectores no cambian; el valor original se conserva en `ORIGINALES_KEY` y es lo que se persiste en "Otros" (guardar la interpretación dejaba la fila sin poder reimportarse). Corpus con baseline escrito a mano: `app/tests/corpus/` — una expectativa que cambia exige subir `revisado_en` y explicarlo.
...
- **Ingesta interactiva para columnas riesgosas** (Sprint 14, reemplazada por F8): `GET /ingestion/files/{id}/preview` expone `columns_at_risk[]` (diagnóstico legacy). `POST /files/{id}/cancel` marca el archivo como `NEEDS_COMPLETION`. El endpoint `POST /files/{id}/drop-columns` de Sprint 14 (mutaba el summary directo, sin `ColumnRiskDecision`, sin cubrir buckets de clientes/proveedores) se **eliminó** — la única ruta autoritativa para columnas riesgosas es el protocolo F8 (`column_risk_decisions` en el confirm, ver bullet siguiente).
- **Protocolo de riesgo contextual de columnas** (F8, `app/application/services/column_risk.py`): un % alto de nulos es DIAGNÓSTICO contextual, nunca decisión automática — ninguna fila va a Otros solo por un campo opcional vacío. `build_contextual_column_risk()` calcula riesgo por columna MAPEADA (no por columna cruda) según la entidad efectiva del contexto; accionable solo si el target es requerido o `user_selected=True`. El usuario decide por columna vía `ColumnRiskDecision` (`drop_column` | `route_affected_rows_to_others`; `cancel_and_complete` es global vía `/cancel`), validadas por `validate_column_risk_decisions()` ANTES del lease (422 si dejarían un requerido sin columna mapeada, o si el par `(source_column, target_field)` declarado no existe en el mapeo efectivo) y aplicadas por `apply_column_risk_decisions()` dentro del savepoint del confirm sobre una COPIA del summary (nunca muta el ORM-tracked original). Huella idempotente propia `risk:{context_id}:{row_index}` (namespace separado de la captura de F6). `GET/POST /files/{id}/column-risk` expone el mismo cálculo read-only para el panel (`ColumnRiskDecisionsPanel.tsx`). La entidad efectiva de un contexto (para requeridos, decisiones de riesgo, custom fields y aprendizaje de mapeos) siempre se resuelve con la MISMA prioridad que la inserción real: override `context_entity` del usuario → entidad original del `mapping_contexts` del summary → `entity_type` del payload — las tres consultas comparten `_entity_for()`/`_override` en el confirm para no divergir entre lo que se valida y lo que se importa.
- **Semántica de precios (F10):** los tres precios de un producto son campos DISTINTOS y coexisten — `unit_cost_ars` (costo unitario vigente/de referencia), `list_price_ars` (sugerido por proveedor/lista, **nuevo**, informativo: NO entra al margen) y `sale_price_ars` (precio de venta vigente que configuró el negocio, NOT NULL, el único del margen). El precio **realmente vendido** vive en la transacción: `sales_entries.unit_price` (**nuevo**, nullable) — cambia por descuento, fecha, canal o cliente, así que no puede vivir solo en `products`. Migración additive `20260809_0001`, sin backfill: `unit_price` NUNCA se deriva de `amount/quantity` (en una fila histórica no se sabe si el monto es unitario o total). `list_price_ars`/`unit_price` se persisten SOLO por mapeo explícito, sin fallback heurístico.
- **Costo facturado ≠ costo de referencia (F-H6.d):** en una compra importada, `inventory_movements.unit_cost` guarda lo que **facturó** el proveedor y `products.unit_cost_ars` el **costo final** (monto ajustado por descuento/impuestos + el flete capitalizado o repartido). Cada lado cae al otro sólo si el archivo no lo trae. El producto declara qué contiene su costo en `custom_fields["_vektor_costo_base"]` (`con_flete` | `sin_flete`; **ausente = "no sé", que NO es `sin_flete`**), y esa procedencia se escribe —y se restaura al borrar el archivo— SIEMPRE junto con el costo. **V5:** `debe_pisar_costo_de_referencia()` (`domain/purchase_cost.py`) impide que una compra facturada pise un costo que incluía flete; un producto sin costo (o en `0`) sí lo recibe, si no el stock quedaría valuado en cero. Consecuencia visible: re-importar compras ya NO actualiza `unit_cost_ars` en todos los casos. El flete que entró al costo lleva `custom_fields["attributed_to_inventory"]` en su `ExpenseEntry` y los agregados de **resultado** lo excluyen (`repositories/_expense_scope.py::gasto_de_resultado`); los de **caja** NO, a propósito. Todo el motor de costos (F-H6.c/d) está detrás de `PURCHASE_COST_ROLLOUT_TENANT_IDS` — ver `docs/runbooks/purchase_cost_rollout.md`.
- **Mapeo que no desempata solo (F10):** `_match_key()` (`column_mapping_service.py`) colapsa preposiciones SOLO para la heurística — `_normalize_col` queda intacta porque es lo que se persiste en `tenant_column_mappings.source_column`. Sin esto, sobre `precio_de_compra` los keywords `precio` (6) y `compra` (6) empataban en longitud y ganaba el orden del dict → el costo entraba como precio de venta (incidente ASTERIA 2026-07-31). **`SINGLE_VALUE_FIELDS`**: campos escalares (montos, cantidades, fechas, los tres precios, stock) donde dos columnas al mismo target se rechazan con 422 — antes `_resolve_target_cols` se quedaba con la primera del orden del Excel y descartaba el resto en silencio. Se evalúa sobre el mapeo EFECTIVO (una columna dropeada por F8 no compite).
- **Catálogo de campos con fuente única (F10):** `GET /ingestion/field-catalog` devuelve `{entity: {required, fields[{value,label,single_value}]}}` derivado de `CANONICAL_FIELDS`/`REQUIRED_FIELDS`/`SINGLE_VALUE_FIELDS`. El frontend NO tiene copia propia: la que existía divergió (le faltaban `payment_method`/`is_recurring` en `expense`) y, como el `<select>` solo renderiza sus opciones, el DOM caía a la primera y la pantalla mostraba "Sin mapear" mientras el estado enviaba otro target. Reglas compartidas en `frontend/src/features/ingestion/mappingRules.ts` (`coversRequired` — un `custom_field:` NO cubre un requerido, espejo de `_missing_required`; `missingRequiredFields`; `scalarCollisions`).
- **`stock_treatment` por hoja (F10):** acepta `{context_id: "opening_balance"|"purchase"}` además del string global (retrocompat). El selector vive dentro de cada `SheetMapperSection` de productos. Un valor único por archivo obligaba a mentir cuando venían juntos un catálogo preexistente y una hoja de compras, y elegir `purchase` generaba COGS por productos ya cargados como egresos del libro diario (doble conteo). Un dict parcial NO contagia: la hoja no declarada cae al default (apertura, el que no toca caja). Aviso no bloqueante cuando una hoja marcada como compra convive con hojas de gastos.
- **Traza del confirm sin huecos (F10):** los rechazos PREVIOS al lease (hojas sin sección, requeridos sin mapear, colisión escalar, gate F6-A1) emiten `STAGE_REJECT` vía `_emit_validation_reject` — antes un 422 no dejaba ni una fila en `pipeline_events`. Es seguro emitir ahí porque la sesión está limpia y no hay lease que compensar (la nota de `_emit_confirm_failure` aplica al camino post-import). El `STAGE_CONFIRM` guarda además el **snapshot del mapeo** (`detail.mappings` por hoja: columna → campo, sin valores de fila) y el `stock_treatment` aplicado: sin eso, saber con qué columna se cargó un precio obligaba a inferirlo de alias aprendidos que no prueban nada sobre un import concreto. Mensajes 422 en castellano con el label de la hoja y la etiqueta del campo.
- **Identidad fuerte de operación y dedup entre archivos (E6b):** la dedup por `(archivo, contexto, índice)` de `operation_fingerprints` sólo ve el MISMO archivo — el mismo contenido subido como archivo nuevo duplicaba todo. `domain/operation_identity.py` define cuándo dos filas SON la misma operación, con la regla «ante la duda, no hay clave» (una clave falsa hace que Véktor descarte plata real): `external_operation_id` **exige** `external_source` declarado; el emisor de una compra es el proveedor por **CUIT/CUIL** (nunca por nombre) y el de una venta propia es el negocio (**el cliente es el receptor**, no entra); tipo y serie se exigen según `FORMATOS_DE_COMPROBANTE` y un tipo desconocido no produce clave; la identidad de LÍNEA no se deriva — sin `external_line_id` la unidad es el **documento completo**, ordenado y preservando repeticiones. La huella de contenido usa los valores EFECTIVOS (mismas funciones que el importador) y las dos representaciones van versionadas (`k1:`/`c1:`). `operation_identity_service.py` reclama con `INSERT ... ON CONFLICT DO NOTHING RETURNING` sobre el UNIQUE `(tenant_id, identity_key)` (mig `20260910_0001`) — **el candado, lo único que resiste dos confirms simultáneos**— antes de escribir efectos, y devuelve al cierre las reclamaciones sin efecto. Decisiones: `ya_aplicada` no inserta y lo avisa; `conflicto` (misma clave, contenido distinto) va a «Otros» y **exige** `aplicar_pese_al_conflicto` para importarse desde ahí (409 `IMPORT_IDENTITY_TAKEN`; el bulk de sugeridos los saltea); `sin_clave` sigue por el circuito de candidatos de `import_overlap_service`. `operation_identity_links` existe para liberar por EFECTO revertido: una reversión parcial NO suelta la identidad. Sin columnas de identidad el confirm paga 0 statements extra; con comprobante completo, 3 fijos (compuerta en `test_ingestion_statement_budget_pg.py`). **Los campos de identidad no se autosugieren** (salvo `invoice_number`): el reconocedor de encabezados lee «Punto de venta» como monto de venta.
- **Envío de compra: mismos efectos entre formatos (E6a-A):** `cobrar_envios_de_la_hoja` dejó de ser un closure del camino multihoja y es función de módulo con `EntornoDeEnvios` explícito; el camino de tabla suelta la llama con la **clave de su hoja** (la descartaba, y por eso las decisiones de costo/envío se validaban, se aceptaban y se ignoraban) y sus avisos llegan a `counts`. **Una tabla única agrupa por comprobante igual que una hoja** — lo determinante son proveedor + número de la fila. El 422 total del archivo plano con costos se acotó a un caso: decisiones sobre mapeos sin `context_id`. Compuerta: `test_paridad_tabla_vs_hoja` compara efectos PERSISTIDOS (gastos, caja, costos, stock, envío una sola vez) entre los dos formatos.
- **Código externo de maestros y productos (E6a-B, `domain/external_code.py`):** `external_code` + `external_source` + `external_code_key` en `customers`/`suppliers`/`products` (mig `20260910_0002`), con **único parcial** `(tenant_id, external_code_key)` excluyendo bajas — el índice tiene que ver lo mismo que la búsqueda (`list_for_dedup`/`is_active`). Ámbito: tenant + tipo de entidad + **sistema de origen** (el "1024" de un sistema no es el de otro). **Los ceros a la izquierda NO se recortan** (`007` ≠ `7`): un código es una cadena — opuesto a `document_series` en `operation_identity`, que sí es un entero. Es clave fuerte de máxima prioridad en `identity_resolution` (maestros) y columna normalizada en `products`; **no se mezclan contratos** — la huella de `operation_identity` no define equivalencia de maestros. La clave se recomputa en `before_insert`/`before_update`, nunca desde el call site. **Identidad ≠ actualización:** `has_user_edits` en customers/suppliers (mig `20260910_0003`) lo marca el PATCH y vuelve el import **aditivo** sobre esa ficha (completa lo vacío, no pisa lo cargado, reporta `preserved_fields`). **En Product, `external_code` es dato adicional, NUNCA clave de matching** (`_resolve_product_identity` sigue sin conocerlo — sigue barcode→SKU→nombre+marca→nombre): el importador lo persiste (aditivo, `_assign_external_code` en `ingestion_import_service.py`) recién DESPUÉS de que la identidad del producto ya está resuelta por otra vía, vía `external_code_guard`/`ProductExternalCodeConflictError` (`product_identity.py`) — deliberadamente separado de `add_product_or_reuse`/`MatchedBy`: un choque de `external_code` entre dos productos ya distinguidos por barcode/sku/nombre NUNCA se resuelve fusionándolos (a diferencia de barcode/sku), se descarta el campo para esa fila y se cuenta (`counts["external_code_conflict"]`). Cubre los 2 caminos de catálogo (tabla suelta y multi-hoja); no aplica a altas desde compra/gasto (`build_incomplete_product`, sin columna de producto que mapear). Integrado con la reversión F11 (`PRODUCT_RESTORE_FIELDS`).
- **Deduplicación acotada al archivo (E6c-1):** `HuellasDelArchivo` + `huellas_presentes` reemplazan a `_load_import_fingerprints` (que traía TODAS las huellas del tenant). Las anclas se enumeran desde `(archivo, contexto, índice)` y se consultan por **lotes de 2.000**. La clase recuerda **qué preguntó**: fuera de ese alcance (el cargo de envío, cuya ancla incluye el monto) hace un `SELECT` y cachea — sin esa distinción, "no está en el set" se confundiría con "nunca la pregunté" y se recobraría un flete ya cobrado. **No es protección de concurrencia**: eso sigue siendo el `ON CONFLICT DO NOTHING` + el lease. Medido: 100 filas contra 100.000 huellas pasó de 32,3 MB a 0,8 MB de pico. Compuerta: `test_las_huellas_se_consultan_acotadas_al_archivo` mira la FORMA del SQL (ninguna consulta sin `IN`), porque contar statements es ciego a esto.
- **Límites de archivo y fórmulas sin calcular (E6c-2, `domain/ingestion_limits.py`):** siete topes + archivo dañado, con motivo específico y acción concreta, verificados en `parse_uploaded_content` (el único punto por el que pasan los 5 caminos que parsean; los 3 del worker leen de S3 y no tocan el chequeo HTTP). **Se rechaza el archivo entero, nunca una parte.** El tamaño expandido se cuenta descomprimiendo por bloques, no leyendo metadatos del ZIP. El reloj se consulta DENTRO del bucle de filas y en la primera llamada (antes cada 512 → un archivo de <512 filas no tenía tope de tiempo). `FORMULA_SIN_RESULTADO` distingue celda vacía / fórmula sin caché / resultado cacheado — `data_only=True` devolvía `None` para las dos primeras y un `.xlsx` de script entraba con las columnas calculadas leídas como vacías. Límite declarado: NO detecta caché desactualizado; cuando no se puede verificar se dice (`formulas_verificadas=False`), nunca se devuelve 0.
- **Importación en segundo plano (E6c-3):** `POST /ingestion/files/{id}/imports` → **202** con `attempt_id`; `GET /ingestion/imports/{attempt_id}` → estado/progreso/resultado/error (verifica tenant, 404 si es de otro). `/confirm` **sigue vigente** durante la transición. Compuerta `ASYNC_IMPORT_ROLLOUT_TENANT_IDS` (vacía = nadie) que gatea **sólo el registro** — el publicador y el recuperador corren siempre, si no las órdenes ya commiteadas quedarían huérfanas. Tablas `import_attempts` + `import_outbox` (mig `20260910_0004`), escritas en la MISMA transacción: el commit es lo único que decide. El intento **congela** la solicitud (`payload_json`), a qué versión del archivo se le dijo que sí (`file_content_hash`) y el formato del sobre (`payload_version`). **El ejecutor llama a `confirm_file`** — no reimplementa el import — para que las dos rutas compartan el lease por archivo. Dos idempotencias distintas: la de PETICIÓN (`request_key`, misma clave+contenido → mismo intento; distinto contenido → 409) y la de EJECUCIÓN (lease con token: la entrega duplicada no reclama). **No se promete "exactly once" de transporte.** Los efectos y el cierre del intento **commitean juntos** (arreglo de la revisión externa): o entran las dos cosas o ninguna, así que una caída en el medio ya no informa fracaso sobre un import que funcionó, y perder el lease revierte los efectos en vez de dejarlos sin dueño. Lo que sigue cubriendo la re-entrega es el token del intento, y las huellas de fila hacen que una segunda corrida no duplique. Frontend: polling con recuperación al recargar; **ante timeout NUNCA cae a `/confirm`** (sólo ante 404), porque la petición pudo haberse registrado. Runbook: `docs/runbooks/async_import_rollout.md`.
- **Cinco agujeros del recorrido asíncrono, cerrados (E6c-3, revisión externa):** (1) el intento congelaba `content_hash` —sha256 de los BYTES— y **una relectura no los toca**, así que el guard era ciego a ella: ahora el ejecutor manda la revisión congelada (`ingestion_version`/`preview_version`) y `confirm_file` la verifica **con el lease ya tomado**, y `reread/apply` **respeta ese lease** (no miraba `processing_status` en absoluto). (2) La recuperación re-arma la orden en su MISMA transacción y no habla con el broker — un broker caído es un motivo habitual de huérfanos, y llamarlo después del commit dejaba intentos sin orden pendiente para siempre. (3) Los efectos y el cierre del intento commitean juntos; obligó a compensar el `IMPORTING` que `acquire_import_lease` commitea a propósito, **en sesión propia** (hacerlo en la que acaba de fallar cuelga el proceso). (4) `liberar_huerfanos` es un UPDATE condicionado con `rowcount` (CAS), como sus dos vecinos; la carrera real se reproduce con **dos recuperadores concurrentes**, no cambiando el estado antes de llamarlo. (5) El `file_id` entra en la comparación de la clave de petición —no en la huella, para no invalidar lo que está en vuelo—: dos archivos con los mismos mapeos daban la misma huella y el segundo recibía el intento del primero, que no es un duplicado sino una **pérdida silenciosa**.
- **Snapshot de capacidades efectivas en el intento (E7a-lite):** `import_attempts.capabilities_json` (mig **`20260910_0005`**) congela al confirmar las cuatro compuertas de rollout que cambian QUÉ se persiste, y el ejecutor las verifica antes de escribir (`ERROR_CAPACIDADES`, no reintentable). Es alcanzable porque la api y el worker son dos servicios con entorno propio que Railway redespliega en paralelo: alguien podía confirmar viendo el motor de costos encendido y el worker escribir con el motor apagado. **Se verifica, no se impone** — imponer exigiría overrides en los ocho lugares donde se leen las compuertas, y una compuerta nueva quedaría afuera afirmando estar congelada. Precio declarado: girar una llave invalida los intentos en vuelo de ese tenant, con mensaje accionable y sin nada escrito. Compuerta de paridad **exhaustiva por contadores** en `test_paridad_tabla_vs_hoja`: el mismo contenido por los dos caminos, comparando TODOS los contadores por diferencia (un contador nuevo entra al alcance solo); única excepción declarada, los `*_ids`, por cardinalidad. Cerró dos divergencias: un gasto sin monto **desaparecía sin rastro** en multihoja mientras el camino plano lo capturaba en «Otros», y `montos_ambiguos` sólo lo publicaba el plano.
- **El camino legacy sin `mapping_contexts` no importa operaciones (E6b, cierre):** ese camino lee por keyword y no tiene mapeo del que derivar la identidad de un comprobante, así que persistía ventas y gastos que nadie podía reconocer como repetidos. No se adivina la clave —una identidad falsa hace que Véktor descarte plata real, peor que el duplicado—: `domain/legacy_import_guard.py` rechaza el confirm con 422 **antes del lease** (traza `STAGE_REJECT`, motivo `legacy_sin_contextos`) y manda a **volver a leer el archivo**, que es lo que lo importa de verdad (la relectura re-importa con el resumen fresco, que sí trae contextos; **no** deja nada para reconfirmar, porque el `parsed_summary_json` guardado no se reescribe). El predicado **espeja el despacho real** de `insert_confirmed_data` —planilla + (`mixed` o `multi_sheet`) + sin contextos + el usuario confirmó ventas/gastos y hay filas—: si divergiera protegería un camino distinto del que escribe. Acotado a operaciones: no toca archivos vacíos, documentos que rutean a «Otros», imports sólo de productos, ni la tabla suelta (que sí planifica identidades). Medido antes de escribirlo: ningún camino de parseo actual produce un summary con operaciones y sin contextos, y en producción **cero archivos** están en ese estado. Consecuencia en los tests: la compuerta de E4 que exigía 200 por ese camino ahora exige el 422, y `ignore` + convenio numérico se miden sobre el bucket de PRODUCTOS, que sigue habilitado y pasa por `preparar_filas_de_hoja`, la misma función.
- **Un documento que sólo deja pendientes no es un import vacío (E8):** en foto/PDF/texto, F6-A4 no extrae fecha, así que cada línea con monto se captura en «Otros» para que el usuario la complete — ese ES el resultado correcto del camino, no un fallback. Pero `check_nonempty_import` contaba como insertado sólo ventas/gastos/productos/maestros, así que ese confirm terminaba en **422 y el rollback del savepoint se llevaba puestas las propias capturas**: el archivo se procesaba, el mensaje hablaba de columnas sin mapear (un documento no tiene columnas) y en la bandeja no quedaba nada. Ahora `counts["otros"]` cuenta como manejo válido **sólo si `file_type` es `text`/`image`** — en una planilla «Otros» sí puede tapar un mapeo que falló, y ahí el 422 sigue siendo lo que deja reintentar (compuerta: una planilla de gastos sin monto, que E7a-lite captura en «Otros», sigue rechazándose). Un documento del que no se pudo leer **un solo monto** sigue siendo vacío: cuenta lo capturado, no lo intentado. El resultado dice lo que pasó —`message` y aviso: «No se importó ninguna venta ni gasto: N línea(s) quedaron pendientes de revisión»— porque el aviso es el único canal que la pantalla renderiza de este resultado (`message` no se muestra). Las capturas de este camino **ya llevan ancla** (`_doc_line_anchor`, namespace propio `doc:`): la identidad es la **ocurrencia** `(archivo, contexto, ordinal dentro del grupo detectado)`, **nunca el texto** —dos renglones idénticos son dos ventas legítimas, y anclar en el contenido descartaría plata real—, más el `row_ref` del ancla de import para que la relectura reconozca lo que ya se clasificó a mano. Sin ancla, cada relectura dejaba otra copia de cada pendiente. Límite declarado: el archivo queda en `DONE` y la lista lo rotula «Importado» —es cierto que se procesó—; lo que quedó pendiente se ve en `/otros` y en el aviso del confirm.
- **El documento fija el convenio numérico, como la columna (E8b):** el camino de foto/PDF/texto era el ÚNICO que leía montos fuera de la política de E4 — no tiene columnas, así que cada token iba a `parsear_monto` sin convenio, que con razón **no adivina** y devuelve `None` para `$12.500`. El llamador leía ese `None` como «la línea no tiene monto» y la línea **desaparecía sin rastro**: ni importada, ni en «Otros», ni contada, ni avisada. Ahora la unidad que resuelve la escala es el **documento** (`inferir_convenio` sobre los tokens de monto de todos sus buckets), con la **misma** regla de evidencia que una columna y no una más laxa: un documento que mezcla `$12.500` con `$12.50` **no** produce convenio y **no se fuerza ninguno**. Cuatro estados por línea, ninguno colapsable (`leer_montos_de_linea`): `sin_token` es texto incidental y NO se materializa —conservar lo ilegible no es convertir «Gracias por su compra» en un pendiente—; `utilizable` se captura como antes; `ambiguo` y `no_positivo` se capturan **con su motivo y el valor original a la vista**, y cada uno con su contador (`lineas_monto_ambiguo` / `lineas_monto_no_positivo`) y su aviso propio, porque no se arreglan igual. `$0` deja de ser indistinguible de la ausencia: la guarda anterior era `any(_parse_amount(...))` y un `Decimal(0)` legítimo es falsy. El ancla de E8 es por ocurrencia y no depende de lo que se haya podido leer, así que los estados nuevos heredan la idempotencia (sincrónico, asíncrono, re-entrega y relectura).
- **Las migraciones del programa son idempotentes (E8c):** el `preDeployCommand` de Railway corre `alembic upgrade head` en CADA deploy, y un esquema que quedó por delante de `alembic_version` lo hace fallar entero. Pasó el 2026-09-12: `alembic_version` decía `20260903_0001` y la columna de `20260906_0001` ya existía → `DuplicateColumn` → deploy abortado (el fail-safe funcionó: `set -e` cortó y la versión vieja siguió sirviendo). Las 7 migraciones del programa (`20260906_0001`, `20260909_0001`, `20260910_0001..0005`) guardan ahora cada creación detrás de `sa.inspect(op.get_bind())`, y el `downgrade` borra sólo lo que está — misma convención que `20260806_0001`, que ya lo documentaba («el `preDeployCommand` puede correr dos veces») y que estas no habían seguido. **Límite declarado: comprueban PRESENCIA, no forma** — una columna que exista con otro tipo se saltea igual; detectar eso pide comparar el esquema entero. Compuerta: `test_migraciones_idempotentes_pg.py` aplica la cadena, **retrocede el stamp sin tocar el esquema** (la deriva real, no "correr upgrade dos veces" —eso alembic lo saltea por versión y no ejecuta nada—) y la vuelve a aplicar; mutar cualquiera de los guards la pone en rojo con el mismo `DuplicateColumn`/`DuplicateTable` de producción. Diagnóstico: `scripts/diag_schema_drift.py` (read-only, lee el `.env` solo) clasifica cada objeto en aplicado/pendiente/DERIVA/FALTA.
- **Eliminación por procedencia de archivo (F11, `file_deletion_service.py`):** borrar un archivo revierte lo que ese archivo creó o modificó, **siempre que no tenga otra fuente válida**, y cuando no puede lo DICE. `DELETE /ingestion/files/{id}` responde **200 con `FileDeletionResult`** (`status`, `fully_reverted`, `deleted`, `restored`, `conservados[]`), nunca 204 mudo; `preview_file_deletion` devuelve la misma forma y **corre la misma lógica** (`_revert_master_items(dry_run=True)`) — pero el DELETE recalcula dentro de su transacción y **su resultado es el autoritativo**: entre preview y borrado el estado puede cambiar. Motivos en `domain/file_deletion_reasons.py` (set cerrado + texto en castellano; el frontend espeja el mapa en `DeleteFileModal.tsx`).
  - **Restauración desde el ledger** (`_ledger_restore.py`, compartido con el undo de relectura): los `UPDATE_PRODUCT`/`UPDATE_CUSTOMER`/`UPDATE_SUPPLIER` vuelven a su `before_json`. Con VARIOS items del mismo archivo sobre una entidad: se restaura el `before` del **primero** (estado previo al archivo) y se compara el `after` del **último** (cómo lo dejó). `stock_units` **NUNCA** por `setattr` — su reversa es exclusivamente `void_movement`.
  - **Guard de edición posterior:** `entity_changed_since_ledger` usa DOS señales y alcanza con una — valores capturados en el `after` (independiente del reloj) **y** `updated_at`. Ninguna sola alcanza: los valores no ven campos fuera del snapshot, y el timestamp no ve nada si dos escrituras caen en el mismo tick.
  - **Ledger de maestros desde el confirm:** `snapshot_masters_before_import` + `build_master_details` (una query por tipo, no `get`+`refresh` por entidad). Acciones `CREATE_CUSTOMER`/`UPDATE_CUSTOMER`/`CREATE_SUPPLIER`/`UPDATE_SUPPLIER` (mig `20260810_0001`); las `REREAD_MASTER_*` son de la relectura y quedan. Los **centinelas** ("Local"/"No identificado") nunca entran al ledger ni se desactivan. El snapshot completo sólo se paga si el archivo TRAE maestros.
  - **Otra fuente = evidencia demostrable** (`_productos_con_otra_fuente`): ventas, compras/gastos, movimientos de otro origen y **otro archivo VIVO** que el ledger diga que también lo trajo. Nunca coincidencia de nombre. `file_id` siempre se excluye del criterio "otro archivo activo" — el archivo que se borra no es evidencia de sí mismo. Lo conservado queda `requires_completion=True`.
  - **Procedencia desde "Otros"** (`others.py`): los 6 destinos dejan rastro. Venta/gasto con `source_upload_id` + `source_row_ref` derivado del `UnclassifiedRecord.id` (estable ante correcciones); maestros y productos en el ledger, porque **no tienen columna de origen**. Incluye la rama de VINCULAR a producto existente, que pisaba precio/costo sin dejar huella.
  - **Derivados post-commit:** `trigger_score_recalculation_after_commit` (`score_trigger_service.py`) encola en el listener `after_commit` — el worker abre su propia sesión y encolarlo antes lo haría leer un estado inexistente. **Fail-safe obligatorio**: ahí la transacción ya commiteó, y propagar un error del broker devolvería un 500 por una operación que sí se completó.
...
- **Reforma de Proveedores (marca ≠ proveedor):** los catálogos de productos ya NO crean proveedores — la marca queda en `Product.custom_fields["marca"]`. Una compra sin proveedor informado se agrupa en un único **sentinela "No identificado"** por tenant (flag `custom_fields["_sentinel"]`, índice único parcial, helper centralizado `is_sentinel_value`/`Supplier.is_sentinel` en `models/supplier.py`). Campos de ficha: `last_name`/`cuil`/`payment_method` (persona o razón social). `inventory_movements.supplier_id` alimenta `GET /suppliers/{id}/products` (`InventoryRepository.products_purchased_from_supplier`: última compra/cantidad/precio unit). Frontend: detalle a **página `/suppliers/[id]`** (no modal), nombre navegable, mailto/WhatsApp, helpers en `frontend/src/lib/suppliers.ts`.
  - **Cambio 4 (en curso, `docs/plans/conservacion-y-acceso-datos-negocio.md`):** mapear "Proveedor — Nombre" (`supplier:name`) con `PRODUCT_SUPPLIER_LINKS_ROLLOUT_TENANT_IDS` apagado ya NO se degrada en silencio a "marca" — se rechaza. Dos capas: (1) rechazo TEMPRANO (422 antes del lease, mensaje con la hoja/columna) en `POST .../confirm` y `POST .../reread/preview` cuando el mapeo se elige EN esa misma llamada (`domain/import_capabilities.contexts_mapping_disabled_supplier_link`, excluye columnas dropeadas por una decisión de riesgo — mapear y dropear la misma columna es un no-op, no un error); (2) chokepoint AUTORITATIVO — `SupplierLinkNotEnabledError`, levantada dentro de `_add_product` (`ingestion_import_service.py`) — es el único punto que ve el mapeo REALMENTE efectivo y cubre los tres caminos que llegan ahí: confirm, la ejecución asíncrona (llama al mismo `confirm_file`) y **la relectura que NO resubmite mapeo** (el caso real: `reread_preview` valida con el flag prendido y deja el draft en el `DataRepairRun`; el `apply` corre en background — Celery — potencialmente mucho después, y si el flag se apagó en el medio, el rechazo temprano ya no protege; solo el chokepoint lo hace). El downgrade silencioso y su contador (`counts["supplier_link_not_enabled"]`) se ELIMINARON — ya no hay camino que los necesite. `GET /ingestion/capabilities` (nuevo, tenant-aware — a diferencia del estático `GET /ingestion/field-catalog`) expone `import_capabilities.capacidades_efectivas()` para que el mapeador pueda deshabilitar el destino antes de confirmar.
  - **Comprobante de importación (`domain/import_receipt.py`):** columna por columna, destino elegido + resultado EFECTIVO (`guardado`/`transformado`/`pendiente`/`excluido`/`mixto`) + motivo + `reason_kind` (`user_decision`/`system_rule`) + `rows_affected` — **nunca** derivado del contador global de la entidad; ausente (`None`) cuando el importador no puede acreditarlo (nunca `0` por default). El inventario parte de `mapping_contexts` COMPLETO (no del mapeo elegido), así que una columna nunca tocada no desaparece — queda "excluido, sin mapeo elegido". `pipeline_events.detail.receipt` (confirm) y `DataRepairRun.details_json["receipt"]` (relectura aplicada) comparten el mismo contrato versionado (`{version, columns[]}`). El `emit_event` de `STAGE_CONFIRM` pasó a `required=True` (nuevo parámetro en `pipeline_event_service.emit_event`, default `False`/fail-silent para el resto de los stages): este evento dejó de ser observabilidad y es el dato — fail-silent ahí dejaría un import "exitoso" sin comprobante. `GET /ingestion/files/{id}/receipt` combina el `STAGE_CONFIRM` más reciente con el `DataRepairRun` REREAD_FILE más reciente, distinguiendo `last_applied` (última ejecución que efectivamente guardó algo) de `last_attempt` (el intento de relectura más reciente aunque haya fallado — un `FAILED` posterior a un confirm exitoso no puede quedar invisible). `file_reverted` viene de `UploadedFile.deleted_at` (F11 nunca toca `pipeline_events`/`data_repair_runs`, así que la explicación sobrevive al borrado). Un evento/run sin `detail.receipt` (anterior a este cambio) se reconstruye SOLO desde `detail.mappings` guardado, marcado `historical_incomplete: true` — nunca se inventa el resto. `undo_reread` marca `run.status="REVERTED"` sin tocar `details_json`: la EJECUCIÓN de relectura queda revertida (visible en `last_attempt` con su comprobante intacto) mientras el ARCHIVO sigue vivo (`file_reverted` sólo refleja `deleted_at`) — son dos estados independientes.
  - **Frontend del comprobante y las capacidades:** `TargetSelect.tsx` acepta `disabledCrossFields: Record<string,string>` — un destino cruzado (hoy solo `supplier:name`) queda visible pero no seleccionable, con el motivo en la etiqueta, cuando `GET /ingestion/capabilities` dice que la compuerta está apagada (mapeo cruzado→compuerta en `mappingRules.disabledCrossFieldReasons`; sin capabilities cargadas todavía no deshabilita nada — mejor dejar elegir que bloquear sobre un estado a medio conocer). `ColumnMapperPanel.tsx` pide capabilities con `useIngestionCapabilities()` (staleTime 5 min, a diferencia del catálogo de campos que es `Infinity`) en los mismos puntos donde ya pedía el catálogo. `FileListSection.tsx` agrega el botón "Comprobante" (mismo gate que "Volver a leer": DONE/NEEDS_COMPLETION) que abre `ImportReceiptModal.tsx`, consumiendo `GET /ingestion/files/{id}/receipt` — muestra `last_applied` y, si existe, `last_attempt` por separado, con el aviso de archivo eliminado cuando corresponde.
...
- **Momento cero del stock de apertura** (`_opening_stock_date()` en `ingestion_import_service.py`): un catálogo tratado como apertura (`stock_treatment` distinto de `purchase`, sin evidencia de compra) ya NO ancla su `occurred_at` en el día de carga (`today`) — usa el primer día real entre las fechas de venta/gasto del propio archivo y las ya cargadas del tenant (`MIN(transaction_date)` sobre `SaleEntry`/`ExpenseEntry` `REAL` no anulados). Sin ninguna fecha real disponible, cae al comportamiento anterior (`tx_date`/`today`) — nunca inventa una fecha. Sigue sin generar `ExpenseEntry` ni tocar caja (invariante de apertura intacta); solo cambia CUÁNDO se declara que ese stock existía. Cierra parcialmente el hueco de cobertura 0 de la reconciliación temporal (bullet anterior) para importaciones NUEVAS — el catálogo ya cargado en un tenant antes de este cambio conserva su fecha `today` vieja salvo backfill puntual. Cubre los 3 caminos de catálogo (tabla suelta, multi-hoja con contexts, legacy sin contexts) — cada uno calcula `_opening_date` a partir de sus propias filas de venta/gasto ya normalizadas (misma función `preparar_filas_de_hoja`/`_parse_date`, nunca un valor crudo). Una compra (`is_purchase=True`) nunca hereda esta fecha — mantiene su propia `tx_date`.
- **Fecha futura rechazada, no importada (`MOTIVO_FECHA_FUTURA` en `domain/date_parsing.py`):** una fecha de negocio (`transaction_date`/`expense_date`/`acquired_at`) que se pudo LEER pero cae después de hoy (`today_ar()`, no un `date.today()` de servidor en UTC) ya no se persiste tal cual — se degrada al mismo tratamiento que una fecha ilegible: la celda se anula (`parsear_fecha_de_columna` la devuelve como si no se pudiera leer) y, en el camino de ventas/gastos de una sola hoja, la fila termina en "Otros" para revisión manual (F6-A2), nunca como una venta o gasto fechado en el futuro. Antes del fix, un valor YA parseado (nativo de openpyxl, serial de Excel, o string sin ambigüedad) sobrevivía sin tocar en la fila y un lector directo más abajo (`_parse_date` sobre un `datetime` nativo no vuelve a filtrar nada) lo reinsertaba igual — el motivo quedaba solo en un contador informativo, no cambiaba lo que se guardaba. Incidente que lo disparó (ASTERIA 2026-09-14): dos celdas de un Libro Diario tipeadas con el año que viene (`26/12/2026` en vez de `26/12/2025`) llevaron 7 ventas reales a diciembre de un año que todavía no había llegado. Alcance deliberadamente acotado: sólo se anula la celda para este motivo nuevo — ambigua/ilegible siguen con su comportamiento de siempre, tocarlo es un cambio aparte. El valor original queda en `ORIGINALES_KEY`, visible en la revisión.
...
- **Planes, suscripción y cupos (Etapa 2 de la política de planes, `docs/plans/planes-suscripcion-y-cupos.md`):** catálogo `plan_definitions` (`FREE` legado + `esencial`/`control`/`direccion`), `Subscription.status` cerrado (`TRIAL`/`ACTIVE`/`GRACE`/`READ_ONLY`/`CANCELLED`, timezone-aware, índice único parcial "una suscripción viva por tenant"). Migraciones: `20260917_0001` (amplía el CHECK de `requested_plan` con los tres planes pagos que ofrece `/precios`) + `20260918_0001` (catálogo, columnas de `subscriptions` y las dos tablas de cupo) + `20260919_0001` (pagos, período siguiente y día ancla), las tres idempotentes. Cupos con reserva/confirmación/liberación atómica (`subscription_quota_usage` + `subscription_quota_reservations`, `app/application/services/subscription_service.py`) — probado bajo concurrencia real contra Postgres. Aprobar una solicitud ahora exige `assigned_plan_code` explícito (nunca infiere de `requested_plan`) y acuña `TRIAL` (nunca `ACTIVE`). `/chat` y `/chat/stream` reservan 1 `ia_query` antes de despachar al orquestador (el rate-limit diario de Redis sigue vigente aparte). Gate transversal `require_active_subscription` (`deps.py`) enganchado en creación de ventas/gastos, nunca en su corrección. Activación manual (cobro por transferencia, sin Mercado Pago todavía) vía `scripts/subscriptions.py` (`show`/`activate`/`set-status`/`expire-due`), idempotente por `--reference`, auditado. **Bloque A (bordes):** `ACTIVE` vence por fecha (gracia con el mismo período → `gracia_vencida`, el MISMO motivo que un `GRACE` vencido: no puede depender de si corrió `expire-due`), FREE legado exento a propósito (también en `expire-due`, que lo pasaba a `GRACE`); `get_current_subscription` ya NO excluye `CANCELLED` (devolvía `None` y los controles lo leían como "sin restricción": cancelar daba IA ilimitada); sin ninguna `Subscription` → **503 `SUBSCRIPTION_UNAVAILABLE`**, nunca acceso libre — **correr `scripts/subscriptions.py diagnose` contra producción antes de desplegar**; `reserve()` devuelve `ReserveOutcome` y solo `NEW` autoriza a ejecutar; `autocommit=False` para componer el cupo con escrituras propias; `enforce_seat_limit` recibe cómo contar (bloquear → contar → crear). Los tenants de los fixtures de test nacen con su FREE, como en producción. **Bloque B (misma política en todos lados):** la autorización es `subscription_service.assert_tenant_can_write()` — función de servicio, porque una dependency de FastAPI no corre cuando el agente o un worker llaman a Python; `require_active_subscription` es solo su traducción a 402/503. Clasificación **por efecto, no por verbo**: gate en las 20 rutas de ALTA (incluye clasificar «Otros», por decisión de producto, y las extracciones foto/PDF, que gastan IA), nunca en `PATCH`/`DELETE`/reactivar/undo; compuerta estructural `test_las_rutas_con_gate_son_exactamente_estas` (`test_subscription_write_gate.py`) — ruta de alta nueva ⇒ agregarla ahí. Agente: un solo embudo, `execute_pending_action`, con lista cerrada `WRITE_EXEMPT_ACTION_TYPES` — **un `ActionType` nuevo nace bloqueado** y hay que clasificarlo (además de `RiskEngine`). `POST /users` aplica plazas (409 `SEAT_LIMIT_EXCEEDED`); FREE legado sin tope. **Bloque C (consumos):** la importación consume ADENTRO de `confirm_file` con `consume_with_data()` (sin commit: entra o se revierte con los datos; chequeo temprano no vinculante para el 429); en segundo plano se reserva al registrar el intento (`import-attempt:{id}`) y el ejecutor confirma ESA reserva vía `quota_operation_id` — **es una dependency y no un parámetro común a propósito: como query param, mandar la clave de una importación ya cobrada haría gratis las siguientes**. La relectura no consume. Foto/PDF: `photo_pdf_read` reserva sólo si el archivo va al modelo y confirma sólo si devolvió datos. Los rechazos que salen como excepción de dominio los traduce el handler global de `main.py` (`subscription_http_error`, una sola traducción). Conciliación: `subscription_reconciliation.py` + `scripts/subscriptions.py reconcile-reservations` + job cada 30 min (Beat sin desplegar ⇒ hoy manual) — **nunca libera sólo por antigüedad**; la clave del chat es `chat:{trace_id}` para poder confirmar contra `decision_audit_log`. Un test que resucite a mano un intento `FALLADO` tiene que reponer su reserva. **Bloque D (servicio comercial, mig `20260919_0001`):** `subscription_billing_service.py` es la ÚNICA puerta por la que se otorga un período pago (hoy el script; después Mercado Pago). **Activar ≠ renovar**: `activate` sólo sin período vigente/en gracia, `renew` sólo con uno — el comando equivocado se rechaza (`USE_RENEW`/`USE_ACTIVATE`); el `activate` viejo pisaba el período en curso. **Pago anticipado** (el caso normal con transferencia) va a `subscriptions.next_*` sin tocar el período ni su saldo, y `effective_access` lo hace regir POR FECHA (`roll_forward` sólo lo deja escrito). Pago en gracia corre desde el vencimiento anterior; meses calendario con `billing_anchor_day` (31 ene → 28 feb → 31 mar), un mes por pago; cambio de plan a la próxima renovación, sin prorrateo, y un downgrade que excede plazas se rechaza; cancelar = `cancel_at_period_end`, corta al fin de lo pago **sin gracia**. Idempotencia por UNIQUE `(source, reference)` en `subscription_payments` (no por búsqueda en la auditoría). `set-status` es reparación: no reinicia pruebas ni acepta `ACTIVE` sin período. **Antes de pushear migraciones: `pytest -m postgres -n 0` contra `vektor_pgtest`** — la compuerta de migraciones idempotentes tenía el head a mano y la suite SQLite no lo veía. Pendiente (bloques E–G de `docs/plans/suscripciones-pendientes-y-cobro.md`): API + pantalla de suscripción (`/auth/me` expone el estado PERSISTIDO, no el efectivo), Mercado Pago (Etapa 3); y, declarados fuera del C, topes de páginas/MB por lectura y el cupo del parseo de imágenes del pipeline de ingesta.
- **B0 del POS — deuda previa del camino de venta (`docs/plans/vektor-pos-scanner-y-ticket.md`):** cuatro arreglos que para una carga manual de a una eran tolerables y para una caja son pérdida de plata o de trazabilidad. **(1) `unit_price` se persiste**: `create_sale` lo aceptaba en el request y nunca lo seteaba, y `manual-batch` lo usaba para calcular `amount` y lo tiraba — sin él un descuento por línea es irrecuperable, porque `amount / quantity` no lo reconstruye (política de `models/transaction.py:57-63`: en una fila histórica no se sabe si el monto es unitario o total). No informado queda `None`, nunca derivado. **(2) Los 7 validadores anti-fecha-futura de `schemas/transaction.py` usan `today_ar()`, no `date.today()`**: el server corre en UTC, 3 h ADELANTE de AR, así que entre las 21:00 y la medianoche argentina `date.today()` ya devuelve mañana y el validador **aceptaba una venta fechada mañana en AR** — permisivo de más, no restrictivo de más, y deja entrar justo lo que `MOTIVO_FECHA_FUTURA` existe para impedir (incidente ASTERIA 2026-09-14). Una caja vende en esa franja todas las noches. Compuerta: `app/tests/domain/test_fecha_negocio_ar.py`. **(3) El rechazo por stock lleva código estructurado**: `InsufficientStockError.CODE == "INSUFFICIENT_STOCK"` y el handler de `main.py` lo emite junto a `product_id`/`available`/`requested` **al lado** de `detail` (que sigue siendo el texto de siempre, para no romper clientes existentes). `manual-batch` dejó de levantar un `HTTPException` suelto con otro shape y levanta la excepción de dominio: los dos caminos responden igual. Hace falta porque el flush de la cola offline tiene que distinguir ESTE 400 de cualquier otro sin leer el copy. El `user_message` ahora nombra el producto y conserva la frase accionable (`INSUFFICIENT_STOCK_ACTION`). **(4) El recálculo de score sale del camino crítico del cobro**: las 5 llamadas de `sales.py` pasaron de `trigger_score_recalculation.delay(...)` —dentro de la transacción, antes del `return`, o sea con Redis caído se caía el cobro— a `trigger_score_recalculation_after_commit(session, ..., background)`, que ya existía y nadie en `sales.py` usaba. **Frontend:** `offlineQueueStore` gana `status: "PENDING" | "FAILED"` (ausente = `PENDING`, para las colas ya persistidas con `version: 1`) y **una operación encolada NUNCA se borra por cantidad de reintentos** — `useOfflineSubmit` llegaba al tope y hacía `remove()`, con un comentario tres líneas arriba que afirmaba lo contrario. Ahora frena en `FAILED`, el flush la saltea (el tope sigue evitando el reintento infinito, que era su razón de ser) y queda visible esperando `retry()` o descarte explícito. **Code review de B0 (7 hallazgos, los 7 verificados contra el código):** (a) el estado terminal nacía **sin salida** — `retry()` no tenía ningún consumidor, así que una carga rechazada quedaba en el badge para siempre; ahora hay `FailedQueuePanel.tsx` dentro del modal de carga manual (reintentar / descartar con confirmación) y el banner cuenta **sólo lo sincronizable**, porque llamar "pendiente de sincronizar" a algo que el flush ya no toca es mentir. (b) **Un error de RED ya no consume presupuesto de reintentos** (`markNetworkFailure`): `navigator.onLine` da `true` con un router sin internet y `flush` corre una vez por navegación de página, así que unos clics durante un corte agotaban el tope y el primer 503 real mandaba la venta a terminal sin haberla reintentado nunca. (c)+(d) **`date.today()` sobrevivía en otros dos validadores anti-futuro** del mismo camino: `schemas/purchase.py` (y `purchase` es un `QueuedKind` de la cola offline) y `ResolvePurchaseRequest` en `others.py` — los dos producen gasto COGS, movimiento de caja y ledger de inventario. Los ~40 `date.today()` restantes son **ventanas de reporte** (`to_date`, `cutoff = hoy − 30d`), no validadores: cambiarlos mueve ventanas de reporte y es otro trabajo. (e) **`update_sale` aceptaba `unit_price` y lo ignoraba**; ahora lo asigna, y si corrigen `amount`/`quantity` **sin** informarlo lo pone en `None` — conservar 500 junto a `amount=800, quantity=2` deja mintiendo al campo cuya razón de ser es decir a qué precio se vendió, y recalcularlo sería la división prohibida. (f) `_sale_snapshot()` no incluía `unit_price`, así que un cambio de precio no dejaba rastro en `decision_audit_log`. (g) Hay **dos formas** de código de error en el backend (`HTTPException(detail={code})` anidado vs. el handler global que deja `detail` como texto y pone `code` al lado); se unificó del lado del LECTOR con `errorCode()` en `useOfflineSubmit.ts`. **Límite declarado: hoy nadie consume `INSUFFICIENT_STOCK` todavía** — el reintento con `capture` es B11, y hasta entonces el flush trata ese 400 como cualquier otro.
- **B2 del POS — idempotencia que devuelve el resultado (mig `20260921_0001`, tabla `idempotency_records`):** `claim_idempotency_key` contesta "esa clave ya se usó" con un `bool` y un **409 vacío**. Eso evita el duplicado, que es la mitad del problema; la otra mitad es la que produce el doble cobro en una caja: **el servidor guardó la venta y el cliente nunca recibió la respuesta** (corte después del commit, timeout del proxy, el navegador cerrado). Un 409 sin cuerpo deja al cajero sin saber si cobró. Y hay una tercera: la misma clave con contenido DISTINTO no es un reintento sino una clave reusada por error, y tragársela hace **desaparecer la segunda venta**. `claim_idempotent_request()` devuelve `Reclamada | Repeticion | ClaveReusada`: misma clave + mismo `request_hash` → **200 con el ticket original** (no 201: el recurso ya existía y esta petición no creó nada); hash distinto → **409 `IDEMPOTENCY_KEY_REUSED`** explícito. El hash se calcula sobre el modelo **ya validado** (`hash_peticion`, `sort_keys`), no sobre los bytes crudos — el orden de las claves del JSON o un espacio no pueden producir un conflicto falso que rechace un reintento legítimo. `record_idempotent_response()` guarda la respuesta en la **MISMA transacción** que las ventas: guardarla después dejaría un ticket persistido cuyo reintento no recupera nada, el mismo agujero corrido un paso. `response_json` nullable a propósito: una ruta que reclama y no registra degrada al 409 de siempre, nunca inventa un cuerpo. El candado es el `UNIQUE (tenant_id, key)` con `guarded_savepoint` —no un SELECT previo—, y en PostgreSQL el savepoint es lo que deja la sesión perdedora **usable** para poder leer el original (sin él, `InFailedSQLTransaction` y un 500 en vez del ticket). Compuerta de concurrencia real: `app/tests/integration/test_idempotencia_concurrencia_pg.py` (dos sesiones con `NullPool`; sin eso comparten conexión y el test pasa vacío). **Alcance: sólo `POST /sales/manual-batch`** — las otras 10 rutas siguen con `claim_idempotency_key`, que no se toca. Las claves viejas NO se migran: un `operation_fingerprint` no tiene ni hash ni respuesta, así que copiarlo daría filas que dicen "ya se usó" sin poder contestar nada, disfrazadas de completas. **Nota de test:** `conftest.override_session` reemplaza `get_db_session` por un `yield` pelado (sin commit ni rollback), así que "el rollback libera la clave" **no es observable por HTTP** — ese caso se prueba a nivel de servicio.
...
- `detect_cost_as_price_imports.py` — detector read-only de productos donde el COSTO pudo entrar como precio de venta (el bug de heurística que arregló F10). Clasifica cada hallazgo por **jerarquía de evidencia**: `1-confirmado` (el snapshot de mapeo del `STAGE_CONFIRM` de ESE archivo lo prueba; solo post-F10), `2-fuerte` (`source_row_ref`/`source_upload_id` + headers reales del `parsed_summary_json` del archivo de origen; cubre el histórico), `3-probable` (alias aprendido en `tenant_column_mappings` + patrón de valores — se reporta como probable, nunca como hecho). Reporta **cobertura** (revisados/sin evidencia/salteados), no solo hallazgos. `--audit` registra `COST_AS_PRICE_IMPORT_FINDING`. NO repara.
...
- **La documentación va en el MISMO commit que el cambio, no al cerrar el bloque.** Regla desde 2026-09-11, y sale de un caso concreto: tres commits seguidos (`7de4a461`, `03ce5518`, `ca5e758c`) no tocaron este archivo, y el resultado no fue "falta documentar" sino **documentación que afirmaba lo contrario del código** —el bullet de E6c-3 seguía declarando como límite conocido una ventana de atomicidad que el arreglo 3 había cerrado— más una **migración sin listar** (`20260910_0005`), que en un deploy se traduce en una columna que el worker escribe y no existe. Un cambio de comportamiento, una migración o un límite que deja de ser cierto se documentan acá en su commit; si el cambio no merece una línea acá, no merece un bullet aparte después.
...
**Migraciones automáticas (Railway pre-deploy):** `backend/railway.toml` define `preDeployCommand = "sh scripts/migrate.sh"`, que corre UNA vez por deploy del servicio `vektor-api`, en un contenedor one-off con `DATABASE_URL` de Neon, ANTES de que la nueva versión reciba tráfico. Tres pasos, en orden: **(1)** `scripts/migrate_preflight.py` — diagnóstico; **(2)** `alembic upgrade head`; **(3)** `scripts/seed_vertical_fields.py` (upsert idempotente de las definiciones de campos por rubro; va después porque un rubro nuevo puede depender de una migración de este mismo deploy). Si (2) o (3) fallan, `set -e` corta y Railway aborta el deploy: la versión vieja sigue sirviendo (fail-safe). El `startCommand` (`start_web.sh`) ya NO corre Alembic — solo uvicorn. `make migrate-neon` queda para aplicar manualmente desde el shell (local contra Neon) cuando haga falta fuera de un deploy.

**Preflight de migración** (`backend/scripts/migrate_preflight.py`): imprime en el log del deploy **contra qué base se va a migrar** — de qué variable salió la URL (`DATABASE_URL_SYNC` le gana a `DATABASE_URL`), host/puerto/base (nunca usuario ni contraseña), `current_database`/`current_user`/`current_schema`/`search_path`, y TODAS las tablas `alembic_version` visibles con sus filas (avisa si hay más de una: leer la versión de un schema y escribir el DDL en otro produce `DuplicateColumn` en loop). Resuelve la URL con el MISMO código que `migrations/env.py` — `app/persistence/db/alembic_url.py`, extraído justamente para que los dos no puedan divergir; si el preflight resolviera por su cuenta podría reportar sobre una base distinta de la que alembic toca. **Nunca corta el deploy:** ante cualquier error sale con 0 y la autoridad sigue siendo alembic. Existe porque un esquema que no coincide con el `alembic_version` que uno ve desde su shell es indistinguible de "otra base", "otro schema" o "log viejo" sin este dato.
