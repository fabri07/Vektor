...
- `reconcile_untagged_adjustments.py` — clasifica adjustments vivos sin `source_type` contra los documentos cargados (dry-run/`--apply`, `--tenant`/`--all-active`); backfillea procedencia o los anula con ajuste incremental de stock; audita `UNTAGGED_ADJUSTMENT_RECONCILIATION`. Desbloquea la migración CHECK `20260728_0001`. Reversa: `revert_untagged_adjustment_reconciliation.py` (aborta si la CHECK ya está aplicada).
