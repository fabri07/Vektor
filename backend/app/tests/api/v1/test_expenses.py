...
class TestExpensesCRUD:
    ...
    async def test_create_expense(self, client: AsyncClient, auth_headers: dict[str, Any]) -> None:
    ...
    async def test_get_expense(self, client: AsyncClient, auth_headers: dict[str, Any]) -> None:
    ...
    async def test_patch_expense(self, client: AsyncClient, auth_headers: dict[str, Any]) -> None:
        create_resp = await client.post(
            "/api/v1/expenses", json=_EXPENSE_PAYLOAD, headers=auth_headers
        )
        expense_id = create_resp.json()["id"]
    ...
    async def test_list_expenses(self, client: AsyncClient, auth_headers: dict[str, Any]) -> None:
...
class TestExpensesTenantIsolation:
    ...
    async def test_cannot_read_other_tenant_expense(
        ...
        resp = await client.get(f"/api/v1/expenses/{expense_id}", headers=second_auth_headers)
...
class TestExpensesRBAC:
    ...
    async def test_viewer_cannot_create_expense(
        ...
        resp = await client.post("/api/v1/expenses", json=_EXPENSE_PAYLOAD, headers=viewer_headers)
