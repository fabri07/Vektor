...
class TestLosConsumidoresSincronicosNoPierdenColumnas:
    """Las tres llamadas REALES que el cableado anterior rompió en silencio.

    No hay pantalla donde desambiguar acá, así que una columna que no mapea es
    una columna perdida: el remito queda sin precio y el proveedor sin nombre. La
    vez pasada el commit afirmaba que estos dos consumidores «no empeoran» y era
    falso — se afirmó sin medirlo. Esto lo mide.
    """

    def test_un_padron_de_proveedores_conserva_el_nombre(self) -> None:
        from app.application.services.supplier_extraction_service import (
            _map_supplier_columns,
        )

        mapeo = _map_supplier_columns(["Proveedor", "Email", "Telefono", "CUIL"])
        assert mapeo.get("Proveedor") == "name"

    def test_un_remito_con_precio_a_secas_conserva_el_precio(self) -> None:
        """«Precio» es ambiguo en un catálogo y NO en un remito: es un documento
        de líneas. Lo declara el que lee el remito, no lo adivina el reconocedor."""
        from app.application.services.remito_extraction_service import _map_columns

        mapeo = _map_columns(["Producto", "Cantidad", "Precio", "Total", "Codigo"])
        assert mapeo.get("Precio") == "unit_price"
        assert mapeo.get("Producto") == "name"

    def test_en_un_remito_la_descripcion_es_el_nombre_del_producto(self) -> None:
        from app.application.services.remito_extraction_service import _map_columns

        mapeo = _map_columns(["Descripcion", "Cant", "Precio unitario", "Importe"])
        assert mapeo.get("Descripcion") == "name"
        assert mapeo.get("Precio unitario") == "unit_price"

    def test_sin_preferencia_declarada_un_ambiguo_no_mapea(self) -> None:
        """La contracara: `prefer` es una decisión explícita del llamador, no un
        default que reintroduzca el adivinar por la ventana."""
        from app.application.services.column_mapping_service import heuristic_target

        assert heuristic_target(_normalize_col("Precio"), "product") is None

    def test_y_solo_puede_elegir_entre_los_candidatos_ofrecidos(self) -> None:
        from app.application.services.column_mapping_service import heuristic_target

        assert heuristic_target(_normalize_col("Precio"), "product", prefer=("notes",)) is None
