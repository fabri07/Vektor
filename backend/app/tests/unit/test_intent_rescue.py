...
def test_rule1_attachment_product_plus_verb():
    assert rescue_intent("analizá esto", True, "product") == "analizar_lista_precios"
...
def test_rule3_attachment_sale_mixed():
    assert rescue_intent("revisá esto", True, "sale") == "analizar_archivo_cargado"
    assert rescue_intent("fijate", True, "mixed") == "analizar_archivo_cargado"


def test_rule4_attachment_no_type_with_verb():
    assert rescue_intent("chequeá esto", True, None) == "analizar_archivo_cargado"


def test_rule5_objects_margen():
    assert (
        rescue_intent("cuánto gano con cada producto", False, None) == "analizar_margenes_productos"
    )
    assert rescue_intent("cuál es mi rentabilidad", False, None) == "analizar_margenes_productos"


def test_rule6_proveedor_aumento_remarcar():
    assert (
        rescue_intent("el proveedor me mandó un aumento", False, None)
        == "detectar_aumentos_proveedor"
    )
    assert rescue_intent("tengo que remarcar todo", False, None) == "simular_actualizacion_precios"
...
def test_rule8_objects_stock():
    assert rescue_intent("qué me falta reponer", False, None) == "detectar_quiebres_stock"
    assert rescue_intent("cómo está mi inventario", False, None) == "detectar_quiebres_stock"
...
def test_rule10_que_puedo_hacer_with_attachment():
    assert (
        rescue_intent("qué puedo hacer con esto", True, None)
...
def test_fuzzy_typos():
    assert rescue_intent("presios de la lista", False, None) == "analizar_lista_precios"
    assert rescue_intent("revisá el stok", False, None) == "detectar_quiebres_stock"
