...
def on_stock_decreased(payload: dict) -> None:
    ...
    with log_job("events.stock_decreased", tenant_id=tenant_id or None, logger=logger):
        if not tenant_id:
...
def on_stock_alert_created(payload: dict) -> None:
    ...
    with log_job("events.stock_alert_created", tenant_id=tenant_id or None, logger=logger):
        logger.warning(
            "stock_alert_created",
            tenant_id=tenant_id,
...
def on_stock_increased(payload: dict) -> None:
    ...
    """
    tenant_id: str = payload.get("tenant_id", "")
    with log_job("events.stock_increased", tenant_id=tenant_id or None, logger=logger):
        if not tenant_id:
...
def on_cash_health_updated(payload: dict) -> None:
    ...
    with log_job("events.cash_health_updated", tenant_id=tenant_id or None, logger=logger):
        if not tenant_id:
...
def on_sale_recorded(payload: dict) -> None:
    ...
    with log_job("events.sale_recorded", tenant_id=tenant_id_str or None, logger=logger):
