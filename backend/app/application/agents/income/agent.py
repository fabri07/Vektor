...
class AgentIncome(BaseAgent):
    ...
    async def _handle_sales_analysis(
        ...
        La familia Clientes se delega a `_handle_customer_analysis` (Fase 2).
        """
        # ── Familia Clientes (Fase 2) — handler real determinístico ───────────
        if intent in (
            "analizar_clientes",
            ...
            return await self._handle_customer_analysis(request, intent)
    ...
    async def _handle_customer_analysis(
        ...
        """Análisis de clientes (Fase 2): mejores clientes, inactivos, cuentas por cobrar.
